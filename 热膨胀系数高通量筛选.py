...
# 获取用户输入的介电常数筛选条件
try:
    dielectric_threshold = float(input("请输入热膨胀系数的筛选值（例如：30）："))
except ValueError:
    print("输入无效，请输入数字。")
    exit(1)

# 按照介电常数的条件进行筛选，去除热膨胀系数的筛选
filtered_df = df[df['CTE'] < dielectric_threshold]


# 随机选择 40 张符合条件的图片
filtered_images = filtered_df.sample(n=40, random_state=21)  # random_state 用于保证结果的可复现性
...
for i, (index, row) in enumerate(filtered_images.iterrows()):
    ...
    axes[i].axis('off')  # 不显示坐标轴
        # 获取性能指标
    dielectric_constant = row['CTE']
    
    # 在图片下方添加性能指标的文字
    axes[i].set_title(f'{image_name}\nCTE: {dielectric_constant}', fontsize=10, pad=10)
